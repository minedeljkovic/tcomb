...
function fromJSON(value, type, path) {
  ...
  switch (kind) {
    ...
    case 'subtype' : // the kind of a refinement is 'subtype' (for legacy reasons)
      ...
      if (true) { // remove optimization
        assert(type.meta.predicate(ret), function () {
          return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/') + ' (expected a valid ' + getTypeName(type) + ')';
        });
      }
      return ret;
    ...
    case 'struct' :
      if (true) { // remove optimization
        assert(isObject(value), function () {
          return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/') + ' (expected an object for type ' + getTypeName(type) + ')';
        });
      }
    ...
    case 'list' :
      if (true) { // remove optimization
        assert(isArray(value), function () {
          return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/') + ' (expected an array for type ' + getTypeName(type) + ')';
        });
      }
    ...
    case 'union' :
      ...
      if (true) { // remove optimization
        assert(isFunction(actualType), function () {
          return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/') + ' (no constructor returned by dispatch of union ' + getTypeName(type) + ')';
        });
      }
    ...
    case 'tuple' :
      ...
      if (true) { // remove optimization
        assert(isArray(value) && value.length === types.length, function () {
          return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/') + ' (expected an array of length ' + types.length + ' for type ' + getTypeName(type) + ')';
        });
      }
      return value.map(function (element, i) {
    ...
    // validate irreducble in production
    case 'irreducible': {
      if (process.env.NODE_ENV === 'production') {
        assert(type.meta.predicate(value), function () { return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/'); });
      }
      return type(value, path);
    }

    // validate enum in production
    case 'enums': {
      if (process.env.NODE_ENV === 'production') {
        assert(type.is(value), function () { return 'Invalid value ' + assert.stringify(value) + ' supplied to ' + path.join('/') + ' (expected one of ' + assert.stringify(Object.keys(type.meta.map)) + ')'; });
      }
      return type(value, path);
    }
